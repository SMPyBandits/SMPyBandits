...
class DiscountedBeta(Beta):
    ...
    def discount(self):
        ...
        """
        self.N[0] = self.gamma * self.N[0]
        self.N[1] = self.gamma * self.N[1]

    def undiscount(self):
        r"""Simply cancel the discount the old observation, when no observation is given at this time.

        .. math::
            \tilde{S}(t+1) &= \frac{1}{\gamma} \tilde{S}(t),
            \tilde{F}(t+1) &= \frac{1}{\gamma} \tilde{F}(t).
        """
        self.N[0] = self.N[0] / self.gamma
        self.N[1] = self.N[1] / self.gamma
