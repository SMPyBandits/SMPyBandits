# -*- coding: utf-8 -*-
""" RandTopM: four proposals for an efficient multi-players learning policy. :class:`RandTopM`, :class:`MCTopM`, :class:`MCTopMCautious` and  :class:`MCTopMExtraCautious`.
...
#: XXX First experimental idea: when the currently chosen arm lies outside of the estimated Mbest set, force to first try (at least once) the arm with lowest UCB indexes in this Mbest_j(t) set. Used by :class:`MCTopMCautious` and :class:`MCTopMExtraCautious`.
OPTIM_PICK_WORST_FIRST = True
OPTIM_PICK_WORST_FIRST = False


#: XXX Second experimental idea: when the currently chosen arm becomes the worst of the estimated Mbest set, leave it (even before it lies outside of Mbest_j(t)). Used by :class:`MCTopMExtraCautious`.
...
class oneRandTopM(ChildPointer):
    ...
    def __init__(self, maxRank,
                 withChair, optimPickWorstFirst, exitIfWorstWasPicked,
                 *args, **kwargs):
        ...
        self._optimPickWorstFirst = optimPickWorstFirst  # FIXME Whether to use or not the variant with the "cautious choice".
        self._exitIfWorstWasPicked = exitIfWorstWasPicked  # FIXME Whether to use or not the second variant with the "even more cautious choice"...
    ...
    def __str__(self):   # Better to recompute it automatically
        ...
        return r"#{}<{}TopM{}-{}{}{}>".format(self.playerId + 1, "MC" if self._withChair else "Rand", ("ExtraCautious" if self._exitIfWorstWasPicked else "Cautious") if self._optimPickWorstFirst else "", player, str_Mbest, str_chosen_arm)
    ...
    def choice(self):
        ...
        if self.t < self.nbArms:  # Force to sample each arm at least one
            ...
            worst_current = self.worst_M_best()
            # FIXME optimization for MCTopMExtraCautious
            if (self.chosen_arm not in current_Mbest) or (self._exitIfWorstWasPicked and self.chosen_arm == worst_current):
                ...
                # FIXME optimization for MCTopMCautious and MCTopMExtraCautious
...
class RandTopM(BaseMPPolicy):
    ...
    def __init__(self, nbPlayers, playerAlgo, nbArms,
                 withChair=False,
                 optimPickWorstFirst=False,
                 exitIfWorstWasPicked=False,
        ...
        for playerId in range(nbPlayers):
            ...
            self.children[playerId] = oneRandTopM(maxRank, withChair, optimPickWorstFirst, exitIfWorstWasPicked, self, playerId)
...
class MCTopM(RandTopM):
    ...
    def __init__(self, nbPlayers, playerAlgo, nbArms,
        ...
        .. warning:: ``s._players`` is for internal use ONLY!
        """
        super(MCTopM, self).__init__(nbPlayers, playerAlgo, nbArms, withChair=True, optimPickWorstFirst=False, exitIfWorstWasPicked=False,maxRank=maxRank, lower=lower, amplitude=amplitude, *args, **kwargs)
...
class MCTopMCautious(RandTopM):
    ...
    def __init__(self, nbPlayers, playerAlgo, nbArms,
        ...
        .. warning:: ``s._players`` is for internal use ONLY!
        """
        super(MCTopMCautious, self).__init__(nbPlayers, playerAlgo, nbArms, withChair=True, optimPickWorstFirst=True, exitIfWorstWasPicked=False, maxRank=maxRank, lower=lower, amplitude=amplitude, *args, **kwargs)
...
class MCTopMExtraCautious(RandTopM):
    ...
    def __init__(self, nbPlayers, playerAlgo, nbArms,
        ...
        >>> s = MCTopMExtraCautious(nbPlayers, Thompson, nbArms)

        - To get a list of usable players, use ``s.children``.

        .. warning:: ``s._players`` is for internal use ONLY!
        """
        super(MCTopMExtraCautious, self).__init__(nbPlayers, playerAlgo, nbArms, withChair=True, optimPickWorstFirst=True, exitIfWorstWasPicked=True, maxRank=maxRank, lower=lower, amplitude=amplitude, *args, **kwargs)
