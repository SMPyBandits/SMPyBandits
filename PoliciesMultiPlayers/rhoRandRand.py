...
class oneRhoRandRand(ChildPointer):
    ...
    def __str__(self):   # Better to recompute it automatically
        if self.rank is not None:
            return '#{}<rhoRandRand, {}, rank:{}>'.format(self.playerId + 1, self.mother._players[self.playerId], self.rank)
        else:
            return '#{}<rhoRandRand, {}>'.format(self.playerId + 1, self.mother._players[self.playerId])
    ...
    def choice(self):
        ...
        result = super(oneRhoRandRand, self).choiceWithRank(1 + rn.randint(self.rank))  # XXX rhoRandRand
        # result = super(oneRhoRandRand, self).choiceWithRank(self.rank)              # XXX rhoRand
